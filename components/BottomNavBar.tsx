...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ currentView, onSetView, onNewNote }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 h-24 bg-transparent z-50 flex justify-center pointer-events-none">
      <div 
        className="absolute w-[calc(100%-2rem)] max-w-sm h-16 bg-gray-900/80 backdrop-blur-lg border border-gray-700/80 rounded-2xl flex items-center justify-around pointer-events-auto"
      ...
      <button
        onClick={onNewNote}
        className="absolute w-16 h-16 bg-blue-500 rounded-full flex items-center justify-center text-white shadow-lg shadow-blue-500/30 hover:bg-blue-600 transition-all duration-300 transform hover:scale-110 focus:outline-none focus:ring-4 focus:ring-blue-500/50 pointer-events-auto"
