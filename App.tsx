...
function App() {
  ...
  const handleCaptureMedia = async (blob: Blob, mimeType: string) => {
    ...
    reader.onloadend = async () => {
        ...
        const name = `${mimeType.split('/')[0]}-${new Date().toISOString()}.${extension}`;
        await saveMedia(blockId, { url: dataUrl, mimeType, name });

        const blockType = mimeType.startsWith('image/') ? ContentBlockType.IMAGE : ContentBlockType.VIDEO;
        const newBlock: ContentBlock = {
            id: blockId,
            type: blockType,
            createdAt: new Date().toISOString(),
            content: { dbKey: blockId, mimeType, name }
        };

        const updatedNote = {
            ...currentActiveNote,
            content: [...currentActiveNote.content, newBlock]
        };
  ...
  const renderView = () => {
    if (currentView === 'note' && activeNote) {
      return <NoteEditor note={activeNote} updateNote={handleUpdateNote} deleteNote={handleDeleteNote} onClose={handleCloseNote} masterPeopleList={masterPeopleList} onAddPersonToMasterList={addPersonToMasterList} onOpenCamera={() => setIsCameraOpen(true)} />;
    }
    switch (currentView) {
      case 'chat':
  ...
  return (
    <div className="h-screen w-screen bg-black text-white flex flex-col font-sans" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
